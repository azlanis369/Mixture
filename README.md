...
- `src/lib/site-config.ts` holds the WhatsApp number for RENFlow Plus
  early-access enquiries, used by every CTA via the shared
  `WhatsAppCta` component (which also fires the analytics events
  documented below).
...
- The Proof section (`src/components/Proof.tsx`) renders the
  ...
  screenshot from `app.renflowplus.com` once an agent has enough data
  to demo (see TODO in that file).
